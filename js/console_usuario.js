...
function traerDatosUsuario() {
    ...
    $.ajax({
        ...
        if (datos.length > 0) {
            ...
            $('#txt_nombre').val(datos[0][6]);
            $('#txt_app').val(datos[0][7]);
            $('#txt_apm').val(datos[0][16]);
            $('#txt_fnac').val(datos[0][15]);
            $('#txt_cel').val(datos[0][13]);
            $('#txt_correo').val(datos[0][8]);
...
function modal_pass() {
    ...
    $("#modal_contra").modal('show');
    let iduser_pass=document.querySelector('#txtprincipalid').value
    document.querySelector('#txt_idusuario_contra').value=iduser_pass
}

function actualizar_imagen_profile() {
    let img_src=document.querySelector('.img-profile').src;
    console.log(img_src);
    let actual=img_src.split('/')
    let name=actual[actual.length-1];
    let usuario = $('#txtprincipalid').val();
    let imagen = $('#img-profile-edit').val();
    if (imagen.length == 0) {
    ...
    var foto = $("#img-profile-edit")[0].files[0];


    formData.append('usuario', usuario)
    formData.append('foto', foto)
    formData.append('img-actual', name)
    formData.append('nombre_archivo', nombre_archivo)

    $.ajax({
        url: '../controller/usuarioC.php?tipo=actualizar_img_profile',
        type: 'post',
        data: formData,
        contentType: false,
        processData: false,
